...
export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema
    .createType("rarity")
    .asEnum(["common", "uncommon", "rare", "epic", "legendary", "ancient"])
    .execute();
  await db.schema
    .createType("assettype")
    .asEnum(["assignment", "character", "item", "resource", "skill"])
    .execute();
  await db.schema
    .createType("droptype")
    .asEnum(["character", "item", "resource", "skill"])
    .execute();
  await db.schema
    .createType("startingtype")
    .asEnum(["character", "item", "resource"])
    .execute();
  ...
  await db.schema
    ...
    .addColumn("address", "varchar(50)", (col) =>
      col.notNull().references("account.address").onDelete("cascade"),
    )
    .addColumn("worldId", "uuid", (col) =>
      col.notNull().references("world.id").onDelete("cascade"),
    )
  ...
  await db.schema
    .createTable("auth")
    .addColumn("address", "varchar(50)", (col) =>
      col.primaryKey().references("account.address").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("assignmentId", "uuid", (col) =>
      col.notNull().references("assignment.id").onDelete("cascade"),
    )
    .addColumn("resourceId", "uuid", (col) =>
      col.references("resource.id").onDelete("cascade"),
    )
    .addColumn("skillId", "uuid", (col) =>
      col.references("skill.id").onDelete("cascade"),
    )
    .addColumn("itemId", "uuid", (col) =>
      col.references("item.id").onDelete("cascade"),
    )
    .addColumn("characterId", "uuid", (col) =>
      col.references("character.id").onDelete("cascade"),
    )
    .execute();
  ...
  await db.schema
    ...
    .addColumn("resourceId", "uuid", (col) =>
      col.references("resource.id").unique().onDelete("cascade"),
    )
    .addColumn("itemId", "uuid", (col) =>
      col.references("item.id").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("skillId", "uuid", (col) =>
      col.notNull().references("skill.id").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("itemId", "uuid", (col) =>
      col.notNull().references("item.id").onDelete("cascade"),
    )
    .addColumn("userId", "uuid", (col) =>
      col.notNull().references("user.id").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("dropTableId", "uuid", (col) =>
      col.notNull().references("dropTable.id").onDelete("cascade"),
    )
    .addColumn("turnHistoryId", "uuid", (col) =>
      col.notNull().references("turnHistory.id").onDelete("cascade"),
    )
    .addColumn("playerCharacterId", "uuid", (col) =>
      col.notNull().references("playerCharacter.id").onDelete("cascade"),
    )
